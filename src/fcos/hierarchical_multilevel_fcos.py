...
class FCOSHead(nn.Module):
    ...
    def __init__(self, in_channels: int, num_anchors: int, num_classes: int, num_sub_classes:int, num_sub_classes_level2:int, num_convs: Optional[int] = 4) -> None:
        super().__init__()
        ...
        # new branch
        self.first_hierarchical_classifier = FCOSClassificationHead(in_channels,  num_anchors, num_sub_classes, num_convs)
        self.second_hierarchical_classifier = FCOSClassificationHead(in_channels,  num_anchors, num_sub_classes_level2, num_convs)
    ...
    def compute_loss(
        ...
        # output of new bransh
        sub_cls_logits = head_outputs["sub_cls_logits"] # [N, HWA, C]
        sub_cls_logits_l2 = head_outputs["sub_cls_logits_l2"] # [N, HWA, C]
        
        #print(sub_cls_logits.shape)
        #print(cls_logits.shape)

        all_gt_classes_targets = []
        all_gt_boxes_targets = []
        all_gt_sub_cls_targets = []
        all_gt_sub_cls_targets_l2 = []
        # steps over each sample in batch
        for targets_per_image, matched_idxs_per_image in zip(targets, matched_idxs):
            # === changed for using empty boxes === #
            if targets_per_image["labels"].all() == 0:
            ...
            # ===================================== #
                ...
                gt_sub_cls_targets = targets_per_image["sub_labels"].new_zeros((len(matched_idxs_per_image),))
                gt_sub_cls_targets_l2 = targets_per_image["sub_labels_l2"].new_zeros((len(matched_idxs_per_image),))
            else:
                gt_classes_targets = targets_per_image["labels"][matched_idxs_per_image.clip(min=0)]
                gt_boxes_targets = targets_per_image["boxes"][matched_idxs_per_image.clip(min=0)]
                gt_sub_cls_targets = targets_per_image["sub_labels"][matched_idxs_per_image.clip(min=0)]
                gt_sub_cls_targets_l2 = targets_per_image["sub_labels_l2"][matched_idxs_per_image.clip(min=0)]

            gt_classes_targets[matched_idxs_per_image < 0] = -1  # backgroud
            # all targets per batch 
            all_gt_classes_targets.append(gt_classes_targets)
            all_gt_boxes_targets.append(gt_boxes_targets)
            all_gt_sub_cls_targets.append(gt_sub_cls_targets)
            all_gt_sub_cls_targets_l2.append(gt_sub_cls_targets_l2)

        all_gt_classes_targets = torch.stack(all_gt_classes_targets)
        all_gt_sub_cls_targets = torch.stack(all_gt_sub_cls_targets)
        all_gt_sub_cls_targets_l2 = torch.stack(all_gt_sub_cls_targets_l2)

        # compute foregroud
        foregroud_mask = all_gt_classes_targets >= 0
        num_foreground = foregroud_mask.sum().item()
        
        # compute foreground for subclassification task (only matched boxes with subclass > 0 count)
        foregroud_mask_subclass = (all_gt_classes_targets >= 0) & (all_gt_sub_cls_targets>=0) # Sub-Class not -1
        num_foreground_subclass = foregroud_mask_subclass.sum().item()
        
        
#        print('Unique subclass_targets:',all_gt_sub_cls_targets.unique(), all_gt_sub_cls_targets.sum())
#        print('Foreground: ',foregroud_mask_subclass.shape, foregroud_mask_subclass.sum())
        foregroud_mask_subclass_l2 = (all_gt_classes_targets >= 0) & (all_gt_sub_cls_targets_l2>=0) # Sub-Class L2 not -1
        num_foreground_subclass_l2 = foregroud_mask_subclass.sum().item()

        # classification loss
        gt_classes_targets = torch.zeros_like(cls_logits)
        # each position which is related to an foreground object gets a 1.0
        # one hot encoding 
        gt_classes_targets[foregroud_mask, all_gt_classes_targets[foregroud_mask]] = 1.0
        #return gt_classes_targets, cls_logits
        loss_cls = sigmoid_focal_loss(cls_logits, gt_classes_targets, reduction="sum")

        # sub_class classification loss (one hot encoding)
        # mask the detections of the sub_class branch with the foreground mask
        gt_sub_cls_targets = torch.zeros_like(sub_cls_logits)
#
        gt_sub_cls_targets[foregroud_mask_subclass, all_gt_sub_cls_targets[foregroud_mask_subclass]] = 1.0

        gt_sub_cls_targets_l2 = torch.zeros_like(sub_cls_logits_l2)
#
        gt_sub_cls_targets_l2[foregroud_mask_subclass_l2, all_gt_sub_cls_targets_l2[foregroud_mask_subclass_l2]] = 1.0
    
#        print('gt_sub_cls_targets',gt_sub_cls_targets.shape, gt_sub_cls_targets.sum(axis=1))
        #return sub_cls_logits, all_gt_sub_cls_targets, foregroud_mask
        loss_sub_cls = sigmoid_focal_loss(sub_cls_logits[foregroud_mask_subclass], gt_sub_cls_targets[foregroud_mask_subclass],reduction="sum")
        # regression loss: GIoU loss

        loss_sub_cls_l2 = sigmoid_focal_loss(sub_cls_logits_l2[foregroud_mask_subclass_l2], gt_sub_cls_targets_l2[foregroud_mask_subclass_l2],reduction="sum")
        ...
        return {
            ...
            "sub_cls": loss_sub_cls / max(1, num_foreground_subclass),
            "sub_cls_l2": loss_sub_cls_l2 / max(1, num_foreground_subclass_l2)
        }

    def forward(self, x: List[Tensor]) -> Dict[str, Tensor]:
        cls_logits = self.classification_head(x)
        bbox_regression, bbox_ctrness = self.regression_head(x)
        # logits for the mitotic figure subclass
        sub_cls_logits = self.first_hierarchical_classifier(x)
        sub_cls_logits_l2 = self.second_hierarchical_classifier(x)
        return {
            "cls_logits": cls_logits,
            "sub_cls_logits_l2": sub_cls_logits_l2,
            "bbox_regression": bbox_regression,
            "bbox_ctrness": bbox_ctrness,
            "sub_cls_logits": sub_cls_logits
        }
...
class FCOSClassificationHead(nn.Module):
    ...
    def forward(self, x: List[Tensor]) -> Tensor:
        ...
        for features in x:
            ...
            # Permute classification output from (Batchsize, Anchors (1) * Classes, H, W) to (N, HWA, K).
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if head is None:
            head = FCOSHead(backbone.out_channels, anchor_generator.num_anchors_per_location()[0], num_classes, num_sub_classes, num_sub_classes_level2)
        self.head = head
    ...
    def eager_outputs(
        ...
        if self.training:
            return losses
        # === added for validation loss computation === #
        elif len(losses) > 0:
    ...
    def postprocess_detections(
        ...
        sub_cls_logits = head_outputs["sub_cls_logits"]
        sub_cls_logits_l2 = head_outputs["sub_cls_logits_l2"]
        ...
        for index in range(num_images):
            ...
            sub_cls_logits_per_image = [sub_cl[index] for sub_cl in sub_cls_logits]
            sub_cls_logits_per_image_l2 = [sub_cl[index] for sub_cl in sub_cls_logits_l2]
            ...
            for box_regression_per_level, logits_per_level, box_ctrness_per_level, anchors_per_level, sub_logits_per_level, sub_logits_per_level_l2 in zip(
                box_regression_per_image, logits_per_image, box_ctrness_per_image, anchors_per_image, sub_cls_logits_per_image, sub_cls_logits_per_image_l2
            ):
                num_classes = logits_per_level.shape[-1]
                num_sub_classes = sub_logits_per_level.shape[-1]
                num_sub_classes_l2 = sub_logits_per_level_l2.shape[-1]
                ...
                # one hot encoded sub_labels wieder in normale labels umrechnen
                sub_labels_per_level = sub_logits_per_level.argmax(dim = -1)
                sub_labels_per_level_l2 = sub_logits_per_level_l2.argmax(dim = -1)
                ...
                image_sub_labels.append(sub_labels_per_level[anchor_idxs])
                image_sub_labels_l2.append(sub_labels_per_level_l2[anchor_idxs])
                image_sub_logits.append(sub_logits_per_level[anchor_idxs])
                image_sub_logits_l2.append(sub_logits_per_level_l2[anchor_idxs])
                
                image_boxes.append(boxes_per_level)
                image_scores.append(scores_per_level)
                image_labels.append(labels_per_level)

            image_sub_labels = torch.cat(image_sub_labels, dim = 0)
            image_sub_labels_l2 = torch.cat(image_sub_labels_l2, dim = 0)
            image_sub_logits = torch.cat(image_sub_logits, dim = 0)
            image_sub_logits_l2 = torch.cat(image_sub_logits_l2, dim = 0)
            ...
            detections.append(
                {
                    ...
                    "sub_labels": image_sub_labels[keep],
                    "sub_labels_l2": image_sub_labels_l2[keep],
                    "sub_logits": image_sub_logits[keep],
                    "sub_logits_l2": image_sub_logits_l2[keep]
                }
            )
    ...
    def forward(
        ...
        # === added loss for validation computation === #
        else:
